...
REQUEST_CONTAINER_2 = endpoints.ResourceContainer(
    message_types.VoidMessage,
    name=messages.StringField(1),
...
class HelloWorldApi(remote.Service):
    ...
    @endpoints.method(REQUEST_CONTAINER, Hello, path = "sayHelloByName", http_method='GET', name = "sayHelloByName")
    def say_hello_by_name(self, request):
      greet = "Hello {}".format(request.name)
      return Hello(greeting=greet)

    @endpoints.method(REQUEST_CONTAINER_2, Hello, path = "greetByPeriod", http_method='GET', name = "greetByPeriod")
